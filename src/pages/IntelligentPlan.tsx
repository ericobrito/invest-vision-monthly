...
import intelligentPlanEngine, { DEFAULT_PLAN_CONFIG } from "@/features/intelligentPlan/intelligentPlanEngine";
...
export default function IntelligentPlan() {
  ...
  // Extract latest snapshot investments
  const latestSnapshot = snapshots.length > 0 ? snapshots[snapshots.length - 1] : null;

  const investmentsData = useMemo(() => {
    if (!latestSnapshot || !latestSnapshot.investments) {
      // Realistic default investment set for single source calculation
      return [
        { name: "META", mode: "DETAILED", appliedBRL: 12000, currentValueBRL: 27000 },
        { name: "GOOGL", mode: "DETAILED", appliedBRL: 14000, currentValueBRL: 18500 },
        { name: "BTC", mode: "DETAILED", appliedBRL: 35000, currentValueBRL: 63845 },
        { name: "ETH", mode: "DETAILED", appliedBRL: 15000, currentValueBRL: 36073 },
        { name: "USDT", mode: "DETAILED", appliedBRL: 50000, currentValueBRL: 51573 },
      ];
    }
    return latestSnapshot.investments.map((inv: any) => ({
      name: inv.name,
      mode: inv.mode || "DETAILED",
      appliedBRL: inv.applied || inv.invested_amount || inv.value || 0,
      currentValueBRL: inv.value || 0,
    }));
  }, [latestSnapshot]);

  // Run Intelligent Plan Engine (Single Source of Truth)
  const analysis = useMemo(() => {
    return intelligentPlanEngine.analyzePortfolio(investmentsData, config);
  }, [investmentsData, config]);

  // Sample crypto disposals for monthly consolidation
  const mockCryptoDisposals = useMemo(() => [
    { id: "1", dateTime: "2026-09-05T10:00:00Z", exchange: "Binance", asset: "BTC", grossProceedsBRL: 20000, gainLossBRL: 8500, taxRegime: "NATIONAL" as const, isExempt: false },
    { id: "2", dateTime: "2026-09-12T14:30:00Z", exchange: "Coinbase", asset: "ETH", grossProceedsBRL: 10000, gainLossBRL: 3200, taxRegime: "NATIONAL" as const, isExempt: false },
  ], []);
  ...
  const simulations = useMemo(() => {
    return intelligentPlanSimulator.simulateScenarios(
      analysis.portfolioMetrics.currentValue,
    ...
    <div className="min-h-screen bg-background text-foreground p-4 sm:p-6 md:p-8 space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-primary/10 text-primary text-xs px-2.5 py-0.5 rounded-full border border-primary/20 font-medium flex items-center gap-1">
              <ShieldCheck className="w-3 h-3" /> Gestão Baseada em Regras Auditáveis
            </span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
            Plano Inteligente <span className="text-primary font-light text-xl">| Execução Estratégica</span>
          </h1>
          <p className="text-xs sm:text-sm text-muted-foreground">
            Transforme dados de carteira em plano acionável de realizações, caixa de oportunidade, reentrada progressiva e otimização tributária.
          </p>
        </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
          <Activity className="w-4 h-4 text-primary" /> 1. Plano Atual — Visão Geral do Portfólio
        </h2>
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-foreground mt-1">
                  R$ {analysis.portfolioMetrics.currentValue.toLocaleString()}
                </h3>
                <p className="text-[11px] text-emerald-500 font-semibold mt-1">
                  +{analysis.portfolioMetrics.profitPercent.toFixed(1)}% de lucro acumulado
                </p>
              </div>
      ...
      {/* SECTION 2: ATENÇÃO AGORA (OPERATIONAL ALERT STATES) */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-amber-500" /> 2. Atenção Agora — Diagnóstico e Regras Acionadas
          </h2>
          <span className="text-xs text-muted-foreground">Regras operacionais configuradas sem predição de mercado</span>
        </div>
          ...
          {analysis.diagnostics.map((item) => (
            <Card key={item.id} className="bg-card border-border hover:border-border/80 transition-all shadow-sm">
              ...
              <CardContent className="p-4 space-y-3 text-xs">
                <p className="text-foreground leading-relaxed bg-muted/30 p-2.5 rounded border border-border">
                  {item.alertMessage}
                </p>
      ...
      {/* SECTION 3: REALIZAÇÕES POSSÍVEIS */}
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
          <DollarSign className="w-4 h-4 text-emerald-500" /> 3. Realizações Possíveis — Preservação de Posição Mínima
        </h2>

        <Card className="bg-card border-border shadow-sm">
          <CardHeader className="p-4 pb-2 border-b border-border">
            <CardTitle className="text-sm text-foreground">Plano Sugerido de Realização Parcial de Lucros</CardTitle>
            <CardDescription className="text-xs text-muted-foreground">
              Sugestão de venda calculada com base no peso máximo e no capital excedente. Nenhuma ordem é executada automaticamente.
            </CardDescription>
          </CardHeader>
              ...
              <table className="w-full text-left text-xs text-foreground">
                <thead className="bg-muted/60 text-muted-foreground font-semibold border-b border-border">
                  <tr>
                    ...
                    <th className="py-3 px-3 text-right">Posição Atual</th>
                    ...
                    <th className="py-3 px-3 text-right">Novo Peso (%)</th>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* SECTION 4: PLANO DE REENTRADA */}
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
            <Zap className="w-4 h-4 text-cyan-500" /> 4. Plano de Reentrada — Escada de Drawdown ({config.reEntryBenchmark})
          </h2>

          <Card className="bg-card border-border shadow-sm space-y-2">
            <CardHeader className="p-4 pb-2 border-b border-border">
              <CardTitle className="text-sm text-foreground">Gatilhos de Reestocagem no Mercado</CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-3 text-xs">
              <div className="space-y-2">
                {analysis.reEntryLadderStatus.map((level) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-lg border font-mono ${
                      level.triggered
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[11px] text-muted-foreground ml-1.5">({level.cashAllocationPct}% do caixa)</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-3">
          ...
          <Card className="bg-card border-border shadow-sm space-y-3 p-4">
            ...
            <div className="p-3 rounded-lg border border-border bg-muted/20 space-y-1.5 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Projeção com Próxima Operação:</span>
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
            <History className="w-4 h-4 text-purple-500" /> 6. Simulador de Estratégias & Avaliação de Desempenho
          </h2>

          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">Cenário de Correção Simulada:</span>
            ...
            <Card key={sim.strategyId} className="bg-card border-border shadow-sm">
              ...
              <CardContent className="p-4 space-y-3 text-xs font-mono">
                ...
                <div className="p-2 rounded bg-muted/40 border border-border text-[11px] font-sans text-muted-foreground">
                  Nota: Simulação hipotética auditável. Não garante rentabilidade futura.
                </div>
              </CardContent>
            </Card>
          ))}
