...
import { Plus, Trash2, Copy, FileX } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
const SnapshotDialog = ({ open, onOpenChange, onSave, snapshot, allSnapshots = [], isSaving }: SnapshotDialogProps) => {
  ...
  return (
    <>
      ...
      <AlertDialog open={showCopyPrompt} onOpenChange={setShowCopyPrompt}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Copiar dados do mês anterior?</AlertDialogTitle>
            <AlertDialogDescription>
              Você deseja copiar os investimentos do mês anterior ({previousSnapshot?.label})?
              Os valores calculados (rentabilidade total e anual) serão recalculados automaticamente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <Button variant="outline" onClick={handleStartEmpty}>
              <FileX className="w-4 h-4 mr-1" /> Não, começar do zero
            </Button>
            <Button onClick={handleCopyPrevious}>
              <Copy className="w-4 h-4 mr-1" /> Sim, copiar
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <DialogContent className="max-w-3xl h-[90vh] flex flex-col">
          ...
          {copiedFromPrev && (
            <div className="bg-primary/10 border border-primary/30 rounded-lg px-4 py-2 text-sm text-primary">
              Dados copiados do mês anterior. Atualize os valores atuais.
            </div>
          )}
